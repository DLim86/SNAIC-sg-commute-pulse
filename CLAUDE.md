...
| `scripts/model.py` | **DONE** (session 8, updated session 11) — `--train` (RandomForestRegressor 11-feat duration + RandomForestClassifier 10-feat crowd; 500 synthetic bootstrap rows each); `--predict` (scores ALL 3 route options **individually** — each gets its own crowd prediction; `prediction_id = f"{option_id}_pred"`); `--evaluate` (7-day MAE); `--backfill` (mode-aware: BUS → LTA alighting stop API, MRT → `total_duration_min + 4` or +20 disruption, LRT → +7/+20, WALK → deterministic). **Session 11 crowd fix:** crowd was previously predicted once for all 3 routes; now per-route with 10 features including `next_bus2_mins`, `bus_headway_gap`, `walk_distance_m`, `num_transfers`, `base_duration`. LTA load snapshots are `actual_crowd` labels only. |
...
| `scripts/scheduler.py` | **DONE** (session 9, updated session 10) — Smart 7-state adaptive pipeline manager replacing the fixed shell loop in docker-compose.yml. States: **NO_EVENT** (60s day / 1hr midnight–6am) → **EVENT_DETECTED_BURST** (5×30s stability before routes fetched) → **WATCHING** (10 min check interval) → **LEAVE_WINDOW** (60s 1s burst then 30s backoff; bus refresh every 60s) → **IN_TRANSIT** (10 min; routes refresh for origin shift) → **ARRIVAL_VERIFY** (5×30s; looking for next event) → **POST_ARRIVAL_COOLDOWN** (10 min cooldown). **Group 1** (`--mode calendar-check`): runs every tick, cached geocode — no OneMap during LEAVE_WINDOW burst. **Group 2** (`--mode routes`): fires only when key changes or event/dest shifts >10m — ip-api + OneMap routing + LTA bus/train + transform + predict + backfill. **Group 3** (`--mode weather`): fires every 30 min regardless of state — weather + transform. |
...
- **`scripts/scheduler.py` state machine (session 9, updated session 10)** — replaces the fixed `sleep 1800` shell loop in docker-compose.yml. **7 states**: NO_EVENT (60s day / 1hr midnight–6am) → EVENT_DETECTED_BURST (5×30s stability before routes are fetched) → WATCHING (calendar every 10 min) → LEAVE_WINDOW (1s burst for 60s then 30s backoff; bus refresh every 60s) → IN_TRANSIT (calendar + routes every 10 min) → ARRIVAL_VERIFY (5×30s looking for next event) → POST_ARRIVAL_COOLDOWN (60s checks for 10 min). **Group 1** `calendar-check` runs every tick; uses cached geocode so no OneMap call during the LEAVE_WINDOW burst. **Group 2** `routes` fires only when `stable_key` changes (event_id or dest shifted >10m). **Group 3** `weather` fires every 30 min independent of state. `docker-compose.yml` pipeline command is `python scripts/scheduler.py`.
...
- **`ingest.py --mode routes` origin refresh** — calls `get_current_location()` (ip-api) fresh on every routes run. ip-api is only called in Group 2 (route refresh), never during the LEAVE_WINDOW 1s burst. Rate limit: 45 req/min free — Group 2 fires at most once per 60s cycle.
...
- Session 9 (2026-06-26): api.py (6 FastAPI endpoints), Airflow DAG (7 tasks + ShortCircuit), Docker (3-service compose), serve.py @st.cache_resource bug fixed, ingest.py 3 argparse modes, scheduler.py state machine (4 states, 3 groups — upgraded to 7 states in session 10).
- Session 10 (2026-06-30): scheduler.py refactored from 4-state to 7-state adaptive machine (NO_EVENT → EVENT_DETECTED_BURST → WATCHING → LEAVE_WINDOW → IN_TRANSIT → ARRIVAL_VERIFY → POST_ARRIVAL_COOLDOWN). All docs updated.
